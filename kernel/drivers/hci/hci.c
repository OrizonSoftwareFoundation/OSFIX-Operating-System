...
int ehci_stop_controller(void) {
    log(Info, "EHCI: stopping controller\n");
    ehci->USBCMD &= ~EHCI_RUN_STOP;

    uint32_t start = get_time_ms();
    while (!(ehci->USBSTS & EHCI_USBSTS_HCHALTED)) {
        if (get_time_ms() - start > 500) {
            log(Info, "EHCI: stop timeout, STS=%08x CMD=%08x, EHCIBASE=%08x\n", ehci->USBSTS, ehci->USBCMD, ehci);
            return -1;
        }
        udelay(1000);
    }

         log(Info, "EHCI: halted successfully, STS=%08x CMD=%08x\n", ehci->USBSTS, ehci->USBCMD);

    return 0;
}

int ehci_reset_controller(void) {
    log(Info, "EHCI: resetting controller\n");

    if (!(ehci->USBSTS & EHCI_USBSTS_HCHALTED)) {
        log(Info, "EHCI: controller not halted, stopping first\n");
        if (ehci_stop_controller() < 0) {
            log(Info, "EHCI: failed to stop before reset\n");
            return -2;
        }
    }
    ...
    log(Info, "EHCI: CMD after set reset=%08x\n", ehci->USBCMD);

    uint32_t start = get_time_ms();
    while (ehci->USBCMD & EHCI_RESET) {
        if (get_time_ms() - start > 1000) {
            log(Info, "EHCI: reset timeout, CMD=%08x STS=%08x, EHCIBASE=%08x\n",
    ...
    log(Info, "EHCI: reset complete, CMD=%08x STS=%08x\n",
...
int ehci_init_simple(void) {
    log(Info, "EHCI init start\n");

    if (ehci_stop_controller() < 0) {
        log(Info, "EHCI stop failed\n");
        return -1;
    }

    log(Info, "EHCI stopped\n");

    if (ehci_reset_controller() < 0) {
        log(Info, "EHCI reset failed\n");
        return -2;
    }

    log(Info, "EHCI reset ok\n");

    if (ehci_init_async_list() < 0) {
        log(Info, "EHCI async list init failed\n");
        return -3;
    }

    log(Info, "Async list ok\n");

    ehci->PERIODICLISTBASE = 0;
    ehci->USBINTR = 0;
    ehci->USBCMD |= (EHCI_ASYNC_EN | EHCI_RUN_STOP);

    log(Info, "EHCI controller run command issued\n");

    uint32_t start = get_time_ms();
    while (!(ehci->USBSTS & (1 << 15))) {
        if (get_time_ms() - start > 100) {
            log(Info, "Timeout waiting for EHCI HCHalted bit clear\n");
            break;
        }
        udelay(100);
    }
    ...
    log(Info, "EHCI USBCMD=0x%x, USBSTS=0x%x\n", cmd, sts);

    log(Info, "EHCI init done\n");
    return 0;
}
...
{
    if (!setup) {
        log(Info, "usb_control_transfer: setup ptr is NULL\n");
        return -1;
    }
    ...
    if (!mem_phys) {
        log(Info, "usb_control_transfer: palloc failed\n");
        return -2;
    }
    ...
    if (ehci_flush_async() != 0) {
        log(Info, "usb_control_transfer: ehci_flush_async failed\n");
    ...
    while ((qh->overlay.token & (1 << 7))) {
        if ((get_time_ms() - start) > 5000) {
            log(Info, "usb_control_transfer: timeout\n");
            async_list_head = old_head;
            pfree(mem_phys);
            return -4;
        }
...
int find_mass_storage_device(usb_device_t *out_dev) {
    ...
    log(Info, "Start 0x%x, end 0x%x", ptr, end);
        ...
        if (type == 0x04 && len >= sizeof(usb_interface_descriptor_t)) {
            ...
            if (found_msd && (ep->bmAttributes & 0x03) == 0x02) {
                if (ep->bEndpointAddress & 0x80) {
                    ...
                    log(Info, "    -> Bulk IN endpoint registered\n");
                } else {
                    ...
                    log(Info, "    -> Bulk OUT endpoint registered\n");
                }
            }

        } else {
            log(Info, "Other descriptor type=%02x len=%u\n", type, len);
        }
    ...
    if (!found_msd) {
        log(Info, "No Mass Storage interface found.\n");
        return -6;
    }
...
void ehci_print_capabilities(void *bar0_virt) {
    ...
    log(Info, "EHCI Capability Registers:\n");
    log(Info, "  CAPLENGTH:  0x%x (%d bytes)\n", caplength, caplength);
    log(Info, "  HCIVERSION: 0x%x (USB %d.%d)\n", 
                   hciversion, (hciversion >> 8) & 0xFF, hciversion & 0xFF);
    log(Info, "  HCSPARAMS:  0x%x\n", hcsparams);
    log(Info, "    N_PORTS:  %d\n", HCSPARAMS_N_PORTS(hcsparams));
    log(Info, "    PPC:      %s\n", HCSPARAMS_PPC(hcsparams) ? "Yes" : "No");
    log(Info, "    N_CC:     %d companion controllers\n", HCSPARAMS_N_CC(hcsparams));
    log(Info, "  HCCPARAMS:  0x%x\n", hccparams);
    log(Info, "  Op Regs at: BAR0 + 0x%x\n", caplength);
...
int hci_init(void) {
    ...
    if (!phys){ 
    log(Warn, "No EHCI MMIO found, *HCI inoperable\n"); 
    return 0; 
}
    ehci_init(phys);
    return 0;
