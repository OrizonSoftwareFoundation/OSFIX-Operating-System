...
#include <io.h>
#include "../storage/includes/sata.h"
#include <ktime.h>
#include <vmm.h>
#include <kprintf.h>
        ...
        if (class_code == 0x01) {
            ...
            if (subclass == 0x01) {
                ...
                if (prog_if == 0x01) {
                    ...
                    log(Info,  "AHCI MMIO base address: %lx\n",
                           (unsigned long)ahci_base);
                    pid_rn = 7;
            ...
            log(Info, "%s Controller: bus=%u dev=%u func=%u vendor id=%x dev id=%x\n",
            ...
            if (prog_if == 0x00) {
                ...
                usb_type = "EHCI";
                log(Info, "EHCI Controller found: bus=%u dev=%u func=%u vendor=%x dev=%x — deferred\n",
            ...
            log(Info, "%s Controller: bus=%u dev=%u func=%u vendor=%x dev=%x\n",
...
void ehci_pci_init(uint8_t bus, uint8_t dev, uint8_t func) {
    ...
    if (bar0 & 0x1) { log(Fatal, "EHCI BAR0 is I/O\n"); return; }
