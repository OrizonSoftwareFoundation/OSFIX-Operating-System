...
void APIC_Initialize(void) {
    ...
    if (!(eax & IA32_APIC_BASE_MSR_ENABLE)) {
        ...
        cpuSetMSR(IA32_APIC_BASE_MSR, eax, edx);
        log(Ok, "Enabled APIC in MSR\n");
    }
    

    cpuGetMSR(IA32_APIC_BASE_MSR, &eax, &edx);
    if (eax & IA32_APIC_BASE_MSR_ENABLE) {
        log(Ok, "APIC enabled and verified in MSR\n");
    } 
    else {
        log(Fatal, "Failed to enable APIC in MSR!\n");
    }
    

    APIC_Write(APIC_SVR, APIC_Read(APIC_SVR) | 0x100 | 0xFF);
    log(Ok, "APIC initialized successfully\n");
