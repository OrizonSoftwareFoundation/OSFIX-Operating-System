...
void vfs_init(void)
{
    registered_count = 0;
    mount_count = 0;
    LOG_OK("VFS successfully initialized\n");
    SERIAL(Ok, vfs_init, "VFS successfully initialized\n");
}

int vfs_register_filesystem(const vfs_filesystem_t *fs)
{
    if (!fs) {
        LOG_FATAL("VFS: cannot register NULL filesystem\n");
        SERIAL(Fatal, vfs_register_filesystem, "VFS: cannot register NULL filesystem\n");
        return -1;
    }
    if (registered_count >= VFS_MAX_FILESYSTEMS) {
        LOG_FATAL("VFS: filesystem registry full\n");
        SERIAL(Fatal, vfs_register_filesystem, "VFS: filesystem registry full\n");
        return -1;
    }
    registered_fs[registered_count++] = fs;
    LOG_INFO("VFS: registered filesystem '%s' (type %u)\n", fs->name, fs->fs_type);
    SERIAL(Info, vfs_register_filesystem, "VFS: registered filesystem '%s' (type %u)\n", fs->name, fs->fs_type);
    return 0;
}
                       ...
                       fs_write_callback write_cb)
{
    ...
    if (!fs) {
        LOG_FATAL("VFS: mount failed - filesystem type %u not registered\n", fs_type);
        SERIAL(Fatal, vfs_mount, "VFS: mount failed - filesystem type %u not registered\n", fs_type);
        return NULL;
    }
    if (mount_count >= VFS_MAX_MOUNTS) {
        LOG_FATAL("VFS: too many mounts\n");
        SERIAL(Fatal, vfs_mount, "VFS: too many mounts\n");
        return NULL;
    }
    ...
    if (!ctx) {
        LOG_FATAL("VFS: filesystem '%s' mount routine failed\n", fs->name);
        SERIAL(Fatal, vfs_mount, "VFS: filesystem '%s' mount routine failed\n", fs->name);
        return NULL;
    }
    ...
    LOG_OK("VFS: Successfully mounted '%s' (type %u)\n", fs->name, fs_type);
    SERIAL(Ok, vfs_mount, "VFS: Successfully mounted '%s' (type %u)\n", fs->name, fs_type);
