...
#include "includes/main/mm/pfndb.h"
...
static void _pfndb_reserve(struct limine_memmap_response *mmap)
{
    ...
    LOG_FATAL("PFNDB: no usable region large enough, need %d bytes\n", (int)size);
    SERIAL(Fatal, _pfndb_reserve, "PFNDB: no usable region large enough\n");
...
void pfndb_init(struct limine_memmap_response *memmap)
{
    ...
    if (!mem_map) {
        LOG_FATAL("PFNDB: translation failed\n");
        SERIAL(Fatal, pfndb_init, "PFNDB: translation failed\n");
        for (;;) __asm__ volatile("hlt");
    }
    ...
    LOG_OK("PFNDB initialized\n");
    SERIAL(Info, pfndb_init, "PFNDB initialized with %d pages\n", (int)max_pfn);
}

void pfndb_dump(void)
{
    LOG_INFO("PFNDB dump start\n");
