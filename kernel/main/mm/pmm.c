...
void pmm_init(void)
{
    if (!hhdm_request.response) {
        SERIAL(Fatal, pmm_init, "PMM: HHDM not available\n");
        LOG_FATAL("PMM: HHDM not available\n");
        for (;;) __asm__ volatile("hlt");
    }
    SERIAL(Info, pmm_init, "HHDM offset: %lx\n", hhdm_request.response->offset);

    if (!memmap_request.response) {
        SERIAL(Fatal, pmm_init, "PMM: memory map not available\n");
        LOG_FATAL("PMM: memory map not available\n");
        for (;;) __asm__ volatile("hlt");
    }
    SERIAL(Info, pmm_init, "Memory map entries: %d\n", memmap_request.response->entry_count);
    ...
    if (!pmm_bitmap) {
        SERIAL(Fatal, pmm_init, "PMM: failed to place bitmap\n");
        LOG_FATAL("PMM: failed to place bitmap\n");
        for (;;) __asm__ volatile("hlt");
    }
    SERIAL(Info, pmm_init, "PMM bitmap placed at physical address %lx, size %lu bytes\n",
    ...
    LOG_OK("PMM initialized: %d bytes total, %d bytes free\n",
