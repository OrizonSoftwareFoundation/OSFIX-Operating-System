...
static void bump_pool_limits(size_t *width_limit, size_t *height_limit) {
    const size_t fixed_bytes =
        ...
        FLANTERM_FB_FONT_GLYPHS * 16 +                    
        FLANTERM_FB_FONT_GLYPHS * 16 * 9 * sizeof(bool) + 
        7 * 16;                                           
...
struct flanterm_context *flanterm_fb_init(
    ...
    if (ansi_colours != NULL) {
        ...
        ctx->ansi_colours[0] = convert_colour(_ctx, 0x00000000);
        ctx->ansi_colours[1] = convert_colour(_ctx, 0x00aa0000);
        ctx->ansi_colours[2] = convert_colour(_ctx, 0x0000aa00);
        ctx->ansi_colours[3] = convert_colour(_ctx, 0x00aa5500);
        ctx->ansi_colours[4] = convert_colour(_ctx, 0x000000aa);
        ctx->ansi_colours[5] = convert_colour(_ctx, 0x00aa00aa);
        ctx->ansi_colours[6] = convert_colour(_ctx, 0x0000aaaa);
        ctx->ansi_colours[7] = convert_colour(_ctx, 0x00aaaaaa);
    }
    ...
    if (ansi_bright_colours != NULL) {
        ...
        ctx->ansi_bright_colours[0] = convert_colour(_ctx, 0x00555555);
        ctx->ansi_bright_colours[1] = convert_colour(_ctx, 0x00ff5555);
        ctx->ansi_bright_colours[2] = convert_colour(_ctx, 0x0055ff55);
        ctx->ansi_bright_colours[3] = convert_colour(_ctx, 0x00ffff55);
        ctx->ansi_bright_colours[4] = convert_colour(_ctx, 0x005555ff);
        ctx->ansi_bright_colours[5] = convert_colour(_ctx, 0x00ff55ff);
        ctx->ansi_bright_colours[6] = convert_colour(_ctx, 0x0055ffff);
        ctx->ansi_bright_colours[7] = convert_colour(_ctx, 0x00ffffff);
    }

    if (default_bg != NULL) {
        ctx->default_bg = convert_colour(_ctx, *default_bg);
    } else {
        ctx->default_bg = 0x00000000;
    }

    if (default_fg != NULL) {
        ctx->default_fg = convert_colour(_ctx, *default_fg);
    } else {
        ctx->default_fg = convert_colour(_ctx, 0x00aaaaaa);
    }
